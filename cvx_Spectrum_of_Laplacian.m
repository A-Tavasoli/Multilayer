clc;clear
%Optimization input
% n=30; m=10; N=n+m; nm=n*m; B0=ones(n,m);
load('A1WSCase1'); load('A2WSCase1'); n=max(size(A1)); m=max(size(A2)); N=n+m; nm=n*m; B0=ones(n,m);
...
cf=100; dc=1; i=0; C=zeros(cf/dc+1,1);  lam_u=zeros(cf/dc+1,N);  lam=zeros(cf/dc+1,N); b1=zeros(cf/dc+1,1);
W=zeros(cf/dc+1,nm); WW=cell(cf/dc+1,1); wSum1=zeros(cf/dc+1,n); wSum2=zeros(cf/dc+1,m); b2=zeros(cf/dc+1,1);
...
% end
i=0; [u,v]=eig(full(L0_2)); v=eig(full(L0_2));
for c=0:dc:cf
    i=i+1;
...
% lam0_1=eig(L0_1); lam0_2=eig(L0_2); Bound0=(1/n+1/m)*C; Bound1=sum(lam0_1)/n+C/n; Bound2=sum(lam0_2)/m+C/m;
...
w=WW{51}; L=[diag(w*um) -w;-w' diag(w'*un)]; L=full(L+L0); [u,v]=eig(full(L));  u=u(:,2); %u=u(1:n);
figure; plot(abs(eigen0_1(:,2)),sum(w,2),'o','Color',[.3 .3 .3])
figure; plot(abs(eigen0_2(:,2)),sum(w,1),'o','Color',[.3 .3 .3])
%% 
w=WW{2}; L=[diag(w*um) -w;-w' diag(w'*un)]; L=full(L+L0);  [u,v]=eig(full(L));  u=u(:,2),
x0=[ones(n,1);-ones(m,1)]; tspan=[0 30]; %x0=rand(N,1);
[t,x] = ode15s(@(t,x) -L*x, tspan, x0); x1=x(:,1:n); x2=x(:,n+1:N);
figure; hold on; plot(t,x1(:,1),'color',[0.47,0.67,0.19],'linewidth',2); plot(t,x2(:,1),'--','color',[0.93,0.63,0.13],'linewidth',2); 
plot(t,x1,'color',[0.47,0.67,0.19],'linewidth',2);  plot(t,x2,'--','color',[0.93,0.63,0.13],'linewidth',2);
xlabel('Time'); ylabel('States of G nodes X(t)');
legend('States of G_1 nodes','States of G_2 nodes')
%% 
% c=10; Bu=(c/nm)*B0; Lu=[diag(Bu*um) -Bu;-Bu' diag(Bu'*un)]; L=full(Lu+L0);
% x0=[zeros(n,1)+1;rand(m,1)]; tspan=[0 20]; %x0=[rand(n,1);-rand(m,1)];
% [t,x] = ode15s(@(t,x) -L*x, tspan, x0); x1=x(:,1:n); x2=x(:,n+1:N);
% figure; hold on; plot(t,x1,'r','linewidth',.1);  plot(t,x2,'b','linewidth',.1);
%% 

% figure; plot(C,e1(2)+max(eigen0_1(:,2).^2)*C,C,lam(:,2))
% for i=1:n
%     for j=1:m
%         def(i,j)=(eigen0_1(i,2)-eigen0_2(j,2))^2;
%     end
% end
% w=WW{6}; figure, plot(vec(def),vec(w),'o');
% k=0;
% for i=1:n
% for j=1:m
% k=k+1;
...
% end
% end
% figure, plot(vec(WW{15}),vec(U),'o')
% w=WW{100}; [u,v]=eig(full(L0_1)); u21=u(:,2); [u,v]=eig(full(L0_2)); u22=u(:,2);
% for i=1:n
%     for j=1:m
...
% [u,v]=eig(full(A1)); u1=u(:,n)*sign(u(1,n));
% [u,v]=eig(full(A2)); u2=u(:,m)*sign(u(1,m));